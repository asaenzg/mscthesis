...
The the null hypothesis and states that both models have the same accuracy. There are three possible alternative hypothesis:
  ...
  2. Less: The second alternative is model 2 being less accurate than model 1
  3. Greater: The last alternative is model 2 being more accurate than model 1
...
We start by setting all H1 to "same". This yields that all models are equally accurate for forecasting inflation in the US.
...
In the case of EA, the test results show that accuracy is not the same among the models when setting the alpha threshold to 0.05. Finally, the test yields that the benchmark outperforms the VAR and that the NN outperforms the benchmark.
...
```{r}
benchmark.mean <- mean(error.benchmark.us)
var.mean <- mean(error.var.us)
nn.mean <- mean(keras.error.us)
...
```{r}
benchmark.mean <- mean(error.benchmark.ea)
var.mean <- mean(error.var.ea)
nn.mean <- mean(keras.error.ea)
...
```{r}
benchmark.mean <- mean(error.benchmark.nl)
var.mean <- mean(error.var.nl)
nn.mean <- mean(keras.error.nl)
...
```{r}
benchmark.mean <- mean(error.benchmark.cl)
var.mean <- mean(error.var.cl)
nn.mean <- mean(keras.error.cl)
...
```{r}
benchmark.mean <- mean(error.benchmark.mx)
var.mean <- mean(error.var.mx)
nn.mean <- mean(keras.error.mx)
